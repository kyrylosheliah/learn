// '*' matches 0 to infinity of arbitrary characters
// '.' matches any single character
// 'a'...'z' match exact symbols in the string
...
fn fuzzyWindowMatching(alloc: *std.mem.Allocator, x: TInput) !TOutput {
    ...
    while (true) {
        ...
        //if (string_overflow) return pattern[i_p] == '*'; // it is handled within the next if statement
        ...
        if (i_p == fuzzy.i_next) {
            ...
            // the pattern was exhausted, but it could match any string ending
            if (i_p == pattern.len) return pattern[pattern.len - 1] == '*';
            fuzzy.i_p = i_p;
        ...
        if (fuzzy.pattern == .start) {
            ...
            'a'...'z' => {
                if (ch == p) {
                    ...
                    fuzzy.i_s += 1;
                    i_s = fuzzy.i_s; // try next string position
                } else return false;
            },
            '.' => {
...
test "10" {
    ...
    try test_suite.inputs.append(.{ .s = "cccab", .p = "*.ca*b", });
    try test_suite.outputs.append(true);
    try test_suite.inputs.append(.{ .s = "cccab", .p = ".*ca*b", });
    try test_suite.outputs.append(true);
    try test_suite.inputs.append(.{ .s = "cccab", .p = ".*.ca*b", });
