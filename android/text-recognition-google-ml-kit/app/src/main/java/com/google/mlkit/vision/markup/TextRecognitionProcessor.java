...
public class TextRecognitionProcessor extends VisionProcessorBase<Text> {
  ...
  public TextRecognitionProcessor(
      Context context, TextRecognizerOptionsInterface textRecognizerOptions, StringBuilder outputAccumulator) {
    this(context, textRecognizerOptions);
    this.accumulator = outputAccumulator;
  }
  ...
  protected void onSuccess(@NonNull Text text, @NonNull GraphicOverlay graphicOverlay) {
    ...
    accumulateClipboardText(text);
    graphicOverlay.add(
        new GraphicOverlay.TextGraphic(
            graphicOverlay,
            text,
  ...
  private void accumulateClipboardText(Text text) {
    if (text != null && accumulator != null) {
      for (int i = 0; i < text.getTextBlocks().size(); ++i) {
        ...
        for (int j = 0; j < lines.size(); ++j) {
          ...
          for (int k = 0; k < elements.size(); ++k) {
            ...
            accumulator.append(element.getText());
            accumulator.append(' ');
          }
          accumulator.append("\n");
        }
