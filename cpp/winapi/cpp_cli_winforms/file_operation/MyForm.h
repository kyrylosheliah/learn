...
#pragma endregion
	...
	private: System::Void DoTask()
	{
		...
		HANDLE hFile = CreateFile(
			...
			NULL,
			OPEN_EXISTING,
			FILE_READ_ONLY, // FILE_FLAG_OVERLAPPED
			NULL);

		if (INVALID_HANDLE_VALUE == hFile)
		{
			label3->Text = "[!] " + textBox1->Text + " не открыт";
			return;
		}

		const DWORD buffer_size = 64;
			...
			NULL))
		{
			...
			// GetOverlappedResult();
			if (dwReadCount) // && (gle != ERROR_ACCESS_DENIED) // ERROR_HANDLE_EOF
			{
				for (i = 0; i < dwReadCount; ++i)
				{
					if (buffer[i] == 32)
					{
						...
						{
							++wordCounter;
							richTextBox1->Text += String::Format("{0} +\t", chCounter);
						}
						else
						{
							richTextBox1->Text += String::Format("{0}\t", chCounter);
